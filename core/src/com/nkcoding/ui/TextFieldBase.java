...
public class TextFieldBase extends Widget implements Disableable {
    ...
    public TextFieldBase (String text, Skin skin, String styleName) {
        this(text, skin.get(styleName, TextField.TextFieldStyle.class));
    }

    public TextFieldBase (String text, TextField.TextFieldStyle style) {
        setStyle(style);
    ...
    void paste (String content, boolean fireChangeEvent, boolean moveCursor) {
        ...
        updateDisplayText();
        if (moveCursor) cursor += content.length();
    }
    ...
    public void next (boolean up) {
        ...
        while (true) {
            TextFieldBase textFieldBase = current.findNextTextField(stage.getActors(), null, bestCoords, currentCoords, up);
            if (textFieldBase == null) { // Try to wrap around.
                if (up)
                    currentCoords.set(-Float.MAX_VALUE, -Float.MAX_VALUE);
                else
                    currentCoords.set(Float.MAX_VALUE, Float.MAX_VALUE);
                textFieldBase = current.findNextTextField(stage.getActors(), null, bestCoords, currentCoords, up);
            }
            if (textFieldBase == null) {
                Gdx.input.setOnscreenKeyboardVisible(false);
                break;
            }
            if (stage.setKeyboardFocus(textFieldBase)) {
                textFieldBase.selectAll();
                break;
            }
            current = textFieldBase;
            currentCoords.set(bestCoords);
        }
    }

    /** @return May be null. */
    private TextFieldBase findNextTextField (Array<Actor> actors, TextFieldBase best, Vector2 bestCoords, Vector2 currentCoords,
                                             boolean up) {
        for (int i = 0, n = actors.size; i < n; i++) {
            Actor actor = actors.get(i);
            if (actor instanceof TextFieldBase) {
                if (actor == this) continue;
                TextFieldBase textFieldBase = (TextFieldBase)actor;
                if (textFieldBase.isDisabled() || !textFieldBase.focusTraversal || !textFieldBase.ancestorsVisible()) continue;
    ...
    public void appendText (String str) {
        ...
        clearSelection();
        cursor = text.length();
        paste(str, programmaticChangeEvents, true);
    }
    ...
    /** The default {@link OnscreenKeyboard} used by all {@link TextFieldBase} instances. Just uses
    ...
    public class TextFieldClickListener extends ClickListener {
        ...
        public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
            ...
            Stage stage = getStage();
            if (stage != null) stage.setKeyboardFocus(TextFieldBase.this);
            keyboard.show(true);
            hasSelection = true;
            return true;
        }
        ...
        public boolean keyDown (InputEvent event, int keycode) {
            ...
            if (ctrl) {
                switch (keycode) {
                    case Keys.V:
                        paste(clipboard.getContents(), true, true);
                        repeat = true;
                        break;
                    case Keys.C:
                    case Keys.INSERT:
            ...
            if ((character == TAB || character == ENTER_ANDROID) && focusTraversal) {
                ...
                if (add || remove) {
                    ...
                    if (add && !remove) {
                        ...
                        if (!enter && filter != null && !filter.acceptChar(TextFieldBase.this, character)) return true;
